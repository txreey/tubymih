        ...
        <div class="flex items-center gap-4">
            ...
            <div class="flex items-center gap-2">
                <div class="text-right hidden sm:block">
                    <div class="text-sm font-semibold leading-tight">{{ auth()->user()->nama ?? 'Owner' }}</div>
                ...
                <div
                    ...
                    {{ auth()->check() && auth()->user()->nama ? strtoupper(substr(auth()->user()->nama, 0, 1)) : 'O' }}
                </div>
            </div>
        </div>
    </header>
                ...
                <nav class="space-y-1">
                    <a href="{{ route('owner.dashboard') }}"
                        class="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors
                              {{ request()->routeIs('owner.dashboard') ? 'bg-teal-800/50 text-teal-300 border-l-4 border-teal-400' : 'text-teal-200/80 hover:bg-teal-900/40 hover:text-white' }}">
            ...
            <div class="mb-8">
                ...
                <nav class="space-y-1">
                    <a href="{{ route('owner.riwayat') }}"
                        class="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors
                              {{ request()->routeIs('owner.riwayat') ? 'bg-teal-800/50 text-teal-300 border-l-4 border-teal-400' : 'text-teal-200/80 hover:bg-teal-900/40 hover:text-white' }}">
                    ...
                    <a href="{{ route('owner.log') }}"
                        class="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors
                              {{ request()->routeIs('owner.log') ? 'bg-teal-800/50 text-teal-300 border-l-4 border-teal-400' : 'text-teal-200/80 hover:bg-teal-900/40 hover:text-white' }}">
