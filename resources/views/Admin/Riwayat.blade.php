...
@section('content')
    <div class="space-y-4 max-w-7xl mx-auto p-4">
        ...
        <div>
            ...
            <p class="text-xs text-gray-500 mt-0.5">Catatan semua transaksi dari kasir — data tetap tersimpan</p>
        </div>
            ...
            <div class="p-4">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                    <div>
                        ...
                        <input type="text" id="filterSearch" placeholder="No transaksi / nama customer..."
    ...
    <script>
        ...
        function renderTable() {
            ...
            pageData.forEach((trx, i) => {
                const rowClass = isTerlambat(trx) ?
                    'bg-orange-50 hover:bg-orange-100 border-l-4 border-l-orange-400' : 'hover:bg-gray-50';
        ...
        function showDetail(id) {
            ...
            let itemsHtml = items.map(item => `
                <div class="flex items-center justify-between py-3 px-4 border-b border-gray-100 last:border-b-0 text-sm hover:bg-gray-50">
                    <div class="flex-1 font-medium text-gray-800 truncate pr-3">${item.nama_menu || item.nama || '-'}</div>
            ...
            document.getElementById('detailContent').innerHTML = `
                <div class="space-y-5">
                    <div class="grid grid-cols-2 gap-4 text-xs bg-gray-50 p-3.5 rounded-xl">
                        <div><span class="text-gray-500">No Transaksi</span><p class="font-mono font-medium text-gray-900">${trx.no_transaksi || '-'}</p></div>
                        <div><span class="text-gray-500">Tanggal</span><p class="text-gray-900">${formatTanggal(trx.created_at || trx.tanggal)}</p></div>
                    </div>
                    <div>
        ...
        function showStruk(id) {
            ...
            const itemsHtml = items.map(item => `
                <div style="display:flex;justify-content:space-between;margin-bottom:8px;">
                    <div style="flex:1;">
                        <p style="margin:0;font-size:13px;font-weight:600;">${item.nama_menu || item.nama}</p>
